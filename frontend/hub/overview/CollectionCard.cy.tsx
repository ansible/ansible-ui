  ...
  it('renders card with expected details', () => {
    cy.fixture('application_collection_versions').then(
      (collectionVersions: HubItemsResponse<CollectionVersionSearch>) => {
        ...
        // Title
        cy.get('div.pf-v6-c-card__title').should('contain', collection.collection_version?.name);
        // Description
        ...
        // Version
        cy.get('div.pf-v6-c-card__body').should(
          'contain',
        ...
        // Signed
        cy.get('.pf-m-green .pf-v6-c-label__content').should('contain', 'Signed');
      }
    );
  });
});
