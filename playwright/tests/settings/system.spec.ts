...
test('system settings edit- can add client id/secret for subscriptions', async ({ page }) => {
  await page.getByRole('button', { name: 'Edit' }).click();
  await page.getByRole('textbox', { name: 'Red Hat Client ID for Subscriptions' }).fill('testId');
  await page
    .getByRole('textbox', { name: 'Red Hat Client Secret for Subscriptions' })
    .fill('testSecret');
  await page.getByRole('button', { name: 'Save' }).click();
  await expect(page.locator('#red-hat-client-id-for-subscriptions')).toContainText('testId');
  if (page.mock.enabled) {
    await expect(page.locator('#red-hat-client-secret-for-subscriptions')).toContainText(
      'testSecret'
    );
  } else {
    await expect(page.locator('#red-hat-client-secret-for-subscriptions')).toContainText(
