...
test.describe('Workflow Viz', () => {
  ...
  test(
    'Add run always nodes and verify that edge, edge label, legend icon, and dropdown option icon are all blue',
    { tag: ['@not_mock', '@compare'] },
    async ({ page }) => {
      test.setTimeout(5 * 60 * 1000);
      const jobTemplateName = createE2EName();
      ...
      await page.getByRole('option', { name: 'Project Sync' }).click();
      await page.getByRole('button', { name: 'Project', exact: true }).click();
      await page.getByRole('textbox', { name: 'Search input' }).fill(project);
      await page.getByRole('option', { name: project }).click();
      await page.getByRole('button', { name: 'Next' }).click();
      await page.getByRole('button', { name: 'Finish' }).click();
      await expect(page.locator('.pf-topology__edge__link').first()).toHaveClass(
        'pf-topology__edge__link'
      );
      await expect(
        page
          .locator('g')
          .filter({ hasText: /^Run always$/ })
          .locator('rect')
          .nth(1)
      ).toHaveCSS('fill', 'rgb(0, 102, 204)');
      await page.locator('.pf-topology__node__action-icon > path').click();
      await expect(
        page.getByRole('menuitem').filter({ hasText: 'Run always' }).locator('path')
      ).toHaveCSS('color', 'rgb(0, 102, 204)');
      await page.getByRole('button', { name: 'Legend' }).click();
      await expect(
        page.getByRole('definition').filter({ hasText: 'Run always' }).locator('path')
      ).toHaveCSS('color', 'rgb(0, 102, 204)');
    }
  );
});
