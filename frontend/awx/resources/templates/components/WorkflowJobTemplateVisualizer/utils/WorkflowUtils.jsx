...
export function getScaleAndOffsetToFit(
  ...
  const scaleNeededForMaxHeight = svgBoundingClientRect.height / gBoundingClientRect.height;
  const scaleNeededForMaxWidth = svgBoundingClientRect.width / gBoundingClientRect.width;
  ...
  if (lowerScale < 0.1 || lowerScale > 2) {
    ...
    yTranslate =
      (svgBoundingClientRect.height - gBoundingClientRect.height * scaleToFit) / 2 -
...
export function getLinePoints(link, nodePositions) {
  const sourceX = nodePositions[link.source.id].x + nodePositions[link.source.id].width + 1;
  let sourceY =
...
export function getLinkOverlayPoints(link, nodePositions) {
  ...
  const pt1 = [
    targetX,
    slope * targetX + yIntercept + orthogonalDistance * Math.sqrt(1 + slope * slope),
  ].join(',');
  const pt2 = [
    sourceX,
    slope * sourceX + yIntercept + orthogonalDistance * Math.sqrt(1 + slope * slope),
  ].join(',');
  const pt3 = [
    sourceX,
    slope * sourceX + yIntercept - orthogonalDistance * Math.sqrt(1 + slope * slope),
...
export function getTranslatePointsForZoom(svgBoundingClientRect, currentScaleAndOffset, newScale) {
  const origScale = currentScaleAndOffset.k;
  const unscaledOffsetX =
    (currentScaleAndOffset.x +
      (svgBoundingClientRect.width * origScale - svgBoundingClientRect.width) / 2) /
    origScale;
  const unscaledOffsetY =
    (currentScaleAndOffset.y +
      (svgBoundingClientRect.height * origScale - svgBoundingClientRect.height) / 2) /
    origScale;
  const translateX =
  ...
  const translateY =
    ...
    (newScale * svgBoundingClientRect.height - svgBoundingClientRect.height) / 2;
