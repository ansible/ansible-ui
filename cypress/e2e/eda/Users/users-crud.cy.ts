...
describe('EDA Users- Create, Edit, Delete', () => {
  ...
  it('can create a User, select role(s) to add the user to, and assert the information showing on the details page', () => {
    ...
    cy.get('[data-cy="username"]').eq(1).type(userInfo.username);
    cy.get('[data-cy="first_name"]').eq(1).type(userInfo.FirstName);
    cy.get('[data-cy="last_name"]').eq(1).type(userInfo.LastName);
    cy.get('[data-cy="email"]').eq(1).type(userInfo.Email);
    cy.get('[data-cy="password"]').eq(1).type(userInfo.Password);
    cy.get('[data-cy="confirmPassword"]').eq(1).type(userInfo.Password);
  ...
  it('can edit a User including the roles the user belongs to', () => {
    cy.createEdaUser({
      ...
      cy.get('[data-cy="username"]').eq(1).type(`${edaUser.username}edited`);
      cy.get('[data-cy="first_name"]').eq(1).type('firstname-edited');
      cy.get('[data-cy="last_name"]').eq(1).type('lastname-edited');
      cy.get('[data-cy="email"]').eq(1).type('edited@redhat.com');
      cy.get('[data-cy="password"]').eq(1).type('newpass');
      cy.get('[data-cy="confirmPassword"]').eq(1).type('newpass');
