...
import { useGetPromptOnLaunchFields, resourceEndPoints } from '../hooks/scheduleHelpers';
    ...
    if (launchConfiguration && launchConfiguration?.ask_instance_groups_on_launch) {
      if (pathname.split('/').includes('job_template')) {
        void getDefaultInstanceGroups(
          `/api/v2/job_templates/${launchConfiguration.job_template_data.id}/instance_groups/`
        );
      }
      if (pathname.split('/').includes('workflow_ job_template')) {
        void getDefaultInstanceGroups(
          `/api/v2/workflow_job_templates/${launchConfiguration.job_template_data.id}/instance_groups/`
        );
      }
    }
