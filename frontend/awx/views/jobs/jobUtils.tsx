...
export function useGetLaunchedByDetails() {
  ...
  const getLaunchedByDetails = useMemo<(job: UnifiedJob) => LaunchedBy>(() => {
    return (job: UnifiedJob) => {
      ...
      const launchedBy = job.launched_by;
      ...
      if (!createdBy && !schedule && !launchedBy) {
        return {};
      }
      ...
      switch (job.launch_type) {
        ...
        case 'dependency':
          value =
            launchedBy?.name === 'Generated by AWX'
              ? t('Generated by Ansible Automation Platform')
              : (launchedBy?.name ?? '');
          link = '';
          break;
        default:
