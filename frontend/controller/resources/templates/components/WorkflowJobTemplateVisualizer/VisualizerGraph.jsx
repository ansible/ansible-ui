...
import WorkflowStartNode from '../WorkflowStartNode.jsx';
...
import VisualizerLink from './VisualizerLink.jsx';
import VisualizerNode from './VisualizerNode.jsx';
import { getScaleAndOffsetToFit } from './utils/WorkflowUtils.jsx';
...
function VisualizerGraph({ readOnly }) {
  ...
  const { addLinkSourceNode, addingLink, links, nodePositions, nodes } =
  ...
  const drawPotentialLinkToNode = (node) => {
    if (node.id !== addLinkSourceNode.id) {
      ...
      const sourceNodeY = nodePositions[addLinkSourceNode.id].y - nodePositions[1].y;
  ...
  //   d3.select(svgRef.current).call(
  //     zoomRef.transform,
  //     d3.zoomIdentity.translate(0, svgBoundingClientRect.height / 2 - 30).scale(1)
  //   );
  ...
  //   const svgBoundingClientRect = svgRef.current.getBoundingClientRect();
  //   const currentScaleAndOffset = d3.zoomTransform(d3.select(svgRef.current).node());
  ...
  const handleFitGraph = () => {
    const { k: currentScale } = d3.zoomTransform(d3.select(svgRef.current).node());
    const gBoundingClientRect = d3.select(gRef.current).node().getBoundingClientRect();
