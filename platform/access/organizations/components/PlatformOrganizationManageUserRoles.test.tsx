    ...
    t: (key: string, options?: Record<string, unknown>) => {
      if (options && typeof key === 'string') {
        ...
        return key.replace(/\{\{(\w+)\}\}/g, (_match, variable: string) => {
          return String(options[variable] || `{{${variable}}}`);
        });
      }
      return key;
    },
...
describe('PlatformOrganizationManageUserRoles', () => {
  const server = setupServer(
    ...
    // Role user assignments endpoint
    ...
    http.post(gatewayAPI`/role_user_assignments/`, () => {
      return HttpResponse.json({ id: 101 }, { status: 201 });
    }),

    // DELETE role assignment
    http.delete(gatewayAPI`/role_user_assignments/100/`, () => {
  ...
  test('should render loading state', () => {
    render(
      <MemoryRouter initialEntries={['/access/organizations/1/users/10/manage-roles']}>
        <Routes>
          <Route
            path="/access/organizations/:id/users/:userId/manage-roles"
    ...
    await waitFor(() => {
      expect(
        screen.getByRole('heading', {
          name: /Manage organization roles directly assigned to test-user1/,
        })
      ).toBeInTheDocument();
    });
