...
describe('PageFromExpandableSection', () => {
  ...
  it('should expand the section when clicked on', () => {
    cy.get('.pf-c-expandable-section__toggle-text').should('contain.text', 'Show advanced options');
    cy.get('#expandable-section').should('not.be.visible');
    cy.get('button').click();
    cy.get('.pf-c-expandable-section__toggle-text').should('contain.text', 'Hide advanced options');
