/** @deprecated Use Schedule from '@ansible/playwright/utils' instead */
...
export async function createJobTemplateSchedule(options: CreateScheduleOptions, page: Page) {
  const inventoryName = options.inventoryName ?? (await createInventory({}, page));
...
export async function deleteSchedule(scheduleName: string, page: Page) {
  await navigateTo(page, 'Automation Execution', 'Schedules');
...
export async function toggleSchedule(
  ...
  if (location === 'details') {
    ...
    const toggleButton = page.getByRole('switch', { name: /Click to (enable|disable) schedule/ });
    await toggleButton.click();
  } else {
    await navigateTo(page, 'Automation Execution', 'Schedules');
    await clickTableRow({ text: scheduleName, filterLabel: 'Name', clearFilters: false }, page);
    const row = page.getByRole('row', { name: new RegExp(scheduleName) });
    const toggleButton = row.getByRole('switch', { name: /Click to (enable|disable) schedule/ });
    await toggleButton.click();
  }
}
