...
describe('EdaDashboard.cy.ts', () => {
  ...
  it('Dashboard renders all components cards', () => {
    ...
    cy.get('[data-cy="There are currently no rulebook activations"]').should('be.visible');
    cy.contains(
    ...
    cy.get('[data-cy="There are currently no decision environments"]').should('be.visible');
    cy.contains(
      'div.pf-c-empty-state__body',
      'Create a decision environment by clicking the button below.'
    );
    cy.contains('h3', 'Rule Audit').scrollIntoView();
    cy.get('[data-cy="There are currently no rule audit records"]').should('be.visible');
    cy.contains('h3', 'Projects').scrollIntoView();
    cy.get('[data-cy="There are currently no projects"]').should('be.visible');
