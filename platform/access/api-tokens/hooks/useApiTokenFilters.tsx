...
export function useApiTokensFilters() {
  const { t } = useTranslation();
  // const searchFilter = useSearchToolbarFilter();
  const queryUsers = useQueryPlatformOptions<PlatformUser, 'username', 'id'>({
  ...
  const queryUserLabel = useCallback(
    (id: string) => <AsyncQueryLabel id={id} url={gatewayAPI`/users/`} field="username" />,
    []
  );
  ...
  const queryApplicationLabel = useCallback(
    (id: string) => <AsyncQueryLabel id={id} url={gatewayAPI`/applications/`} />,
    []
  );
