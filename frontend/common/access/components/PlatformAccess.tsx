...
type PlatformAccessProps<T extends Assignment> = {
...
export function PlatformAccess<T extends Assignment>(props: PlatformAccessProps<T>) {
  ...
  function useRemoveRoles(onComplete: (roles: T[]) => void) {
    ...
    const bulkAction = useBulkConfirmation<T>();
  ...
  const queryParams = useMemo<QueryParams>(() => {
    ...
    switch (props.accessListType) {
      case 'user':
      case 'team':
        params = { object_id: props.id, content_type__api_slug: props.content_type_model };
        break;
      case 'user-roles':
    ...
    <PageTable
      id={
        props.content_type_model
          ? `${props.content_type_model}-access-table`
          : `${props.accessListType}-table`
      }
