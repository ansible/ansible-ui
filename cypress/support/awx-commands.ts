...
// ***********************
// CREATE HOSTS CMD HERE

/**
 * Custom command to navigate the UI to create a host OR an inventory host
 * 
 * - Goes to the UI path for host OR inventory host
 * - Verify it's in the create host page and add data to input fields
 * - TODO: explain what else
 * 
 * @param isInventoryHost - If this is inventory host or not
 * @param inventory - String with inventory name to be searched in the table
 * @example Create Host in UI
 * ```
 * cy.createHostOrInvHostWithUI(false, '')
 * cy.createHostOrInvHostWithUI(true, 'E2E Inventory cFdd')
 * ```
 */
Cypress.Commands.add('createHostOrInvHostWithUI', (isInventoryHost: boolean, inventory: string) => {
...
Cypress.Commands.add('verifyHostOrInvHostDataWithUI', (hostName: string, inventory_host: boolean, invenotry: string) => {

    navigateToHostOrInvHostWithUI(inventory_host, hostName, invenotry, '[data-cy="name-column-cell"] a');
...
Cypress.Commands.add('naviateToHostOrInvHostWithUI', (inventory_host: boolean, name: string, invenotry: string, data: string) => {
    let url = '';
    if (inventory_host) {
...
Cypress.Commands.add(
  'editHostOrInvHostWithUI',
  (inventoryID: number, inventory_host: boolean, hostName: string) => {
