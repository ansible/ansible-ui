...
import { EdaCredentialTypeAddTeams } from './EdaCredentialTypeAddTeams';

describe('EdaCredentialTypeAddTeams', () => {
  const component = <EdaCredentialTypeAddTeams />;
  const path = '/credential-types/:id/team-access/add-teams';
  const initialEntries = [`/credential-types/1/team-access/add-teams`];
  const params = {
    path,
    initialEntries,
  };
  ...
  it('should trigger bulk action dialog on submit', () => {
    cy.intercept('POST', edaAPI`/role_team_assignments/`, {
      statusCode: 201,
      body: { team: 3, role_definition: 14, content_type: 'eda.credentialtype', object_id: 1 },
