...
export function PageFormMultiSelectEdaResource<
  ...
  const queryOptions = useCallback<PageAsyncSelectOptionsFn<PathValue<FormData, Name>>>(
    async (options) => {
      try {
        ...
        urlSearchParams.set('page_size', options?.next ? `${options.next}` : '10');
          ...
          for (const [key, value] of Object.entries(props.queryParams)) {
            if (Array.isArray(value)) {
              for (const subValue of value) {
                urlSearchParams.set(key, subValue);
              }
            } else {
        ...
        if (options.search) urlSearchParams.set('name', options.search);
