...
We have the `main` branch and the `release/2.5` branch. In the future, we might have more branches for other releases.

Not all fixes added to the `main` branch are backported to the `release/2.5` branch or to other branches.

For example, if a customer reports a bug in the `2.5` release, we fix it in the `main` branch, then backport the fix to `release/2.5` so we can provide a fast resolution for the customer.

## Criteria to Backport a Fix

- **Customer-reported issues (2.5)**: If an issue is reported by a customer using `2.5`, we should backport it, since it directly affects them. We need to address the issue on the main branch first, then backport it to the `release/2.5` branch.
