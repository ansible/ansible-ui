...
import { useDeleteOAuthApplications } from './useDeleteOAuthApplications';

export function useOAuthApplicationActions(options: {
  onApplicationsDeleted: (applications: Application[]) => void;
}) {
  const { activePlatformUser } = usePlatformActiveUser();
  const { onApplicationsDeleted } = options;
  const { t } = useTranslation();
  const deleteApplications = useDeleteOAuthApplications(onApplicationsDeleted);
