...
export function useTasksColumns(_options?: { disableSort?: boolean; disableLinks?: boolean }) {
    ...
    const tableColumns = useMemo<ITableColumn<Task>[]>(
        () => [
            ...
            { header: t('Created'), cell: (task) => <SinceCell value={task.pulp_created} /> },
            { header: t('Started'), cell: (task) => <SinceCell value={task.started_at} /> },
            { header: t('Finished'), cell: (task) => <SinceCell value={task.finished_at} /> },
            { header: t('Status'), cell: (task) => <StatusCell status={task.state} /> },
        ],
        [t]
    )
    return tableColumns
