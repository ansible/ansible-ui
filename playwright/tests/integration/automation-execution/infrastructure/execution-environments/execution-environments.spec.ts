...
test.describe('Execution Environments', () => {
  test.describe('Execution Environments: Create', () => {
    ...
    test.afterEach(async ({ page }) => {
      await Organization.api.deleteByName(page, organization.name).catch(() => {});
    });
    ...
    test(
      ...
      async ({ page }) => {
        ...
        await expect(page.locator('#organization')).toContainText(organization.name);
    ...
    test(
      ...
      async ({ page }) => {
        ...
        await page.locator('#organization').getByRole('link', { name: organization.name }).click();
    ...
    test(
      ...
      { tag: ['@not_mock'] },
      async ({ page }) => {
        const user = await User.api.create(page, { password: 'testPassword123!' });
        const execEnvName = createE2EName('exec-env');
        const image = 'quay.io/ansible/awx-ee:latest';

        await Organization.ui.addUser(page, organization.name, user.username, {
        ...
        await logout(page);
        await login(page, platformUI, { username: user.username, password: 'testPassword123!' });
        ...
        await logout(page, { username: user.username });
        await login(page);
  ...
  test.describe('Execution Environments: Templates View', () => {
    ...
    test(
      ...
      async ({ page }) => {
        ...
        await page.getByRole('textbox', { name: 'Search input' }).fill(inventory.name);
        await page.getByRole('option').filter({ hasText: inventory.name }).click();
        ...
        await expect(page.locator('#inventory')).toContainText(inventory.name);
