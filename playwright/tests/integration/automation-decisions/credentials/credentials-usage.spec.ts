...
test.describe(
  'EDA Credentials Usage in Resources',
  { tag: ['@not_mock'] },
  () => {
    ...
    test.beforeEach(async ({ page }) => {
      ...
      const platformOrg = await gatewayAPI.post<PlatformOrganization>(
        page,
        'organizations/',
        {
      ...
      const edaOrganization = await EdaOrganization.api.getByAnsibleId(
        page,
        ansibleId,
      );
      ...
      edaDecisionEnvironment = (await edaAPI.post(
        page,
    ...
    test.afterEach(async ({ page }) => {
      ...
      if (edaDecisionEnvironment?.id) {
        await edaAPI
          .delete(
            page,
            `decision-environments/${edaDecisionEnvironment.id}/?force=true`,
          )
          .catch(() => {});
      }
      if (platformOrgId) {
        await gatewayAPI
          .delete(page, `organizations/${platformOrgId}/`)
          .catch(() => {});
      }
    });
    ...
    test('should create RBA without credentials', async ({ page }) => {
      ...
      await expect(
        page.getByRole('heading', { name: 'Create rulebook activation' }),
      ).toBeVisible();
      ...
      await page
        .locator('#decision_environment_id-search input')
        .fill(edaDecisionEnvironment.name);
      await page
        .getByRole('option', { name: edaDecisionEnvironment.name })
        .click();
      ...
      const createResponsePromise = page.waitForResponse(
        (response) =>
          response.url().includes('/api/eda/v1/activations/') &&
          response.status() === 201,
      );
      ...
      try {
        await expect(
          page.getByRole('heading', { name: rbaName, exact: true }),
        ).toBeVisible();

        await navigateTo(page, 'Automation Decisions', 'Rulebook Activations');
        await page
          .getByRole('textbox', { name: 'Type to filter' })
          .fill(createdRBA.name);
        ...
        await page
          ...
          .click();
        await page
          .getByRole('menuitem', { name: 'Restart rulebook activation' })
          .click();
        ...
        await page
          .getByRole('button', { name: 'Restart rulebook activations' })
          .click();
        ...
        if (createdRBA?.id) {
          await edaAPI
            .delete(page, `activations/${createdRBA.id}/`)
            .catch(() => {});
        }
      }
    });

    test('should not create a private project without credentials', async ({
      page,
    }) => {
      test.setTimeout(180000);
      ...
      await page
        ...
        .fill('https://github.com/ansible/ansible-ui');
      ...
      const createResponsePromise = page.waitForResponse(
        (response) =>
          response.url().includes('/api/eda/v1/projects/') &&
          response.status() === 201,
      );
      ...
      try {
        ...
        if (newProject?.id) {
          await edaAPI
            .delete(page, `projects/${newProject.id}/`)
            .catch(() => {});
        }
      }
    });

    test('should not use a private DE without credentials', async ({
      page,
    }) => {
      test.setTimeout(180000);
      ...
      try {
        ...
        await expect(
          page.getByRole('heading', { name: 'Decision Environments' }),
        ).toBeVisible();
        ...
        await page
          .getByLabel('Image')
          .fill('quay.io/abakshirht/galaxy-ng-locust:ansible2.13');

        const createDEResponsePromise = page.waitForResponse(
          (response) =>
            response.url().includes('/api/eda/v1/decision-environments/') &&
            response.status() === 201,
        );
        ...
        const privateDE =
          (await createDEResponse.json()) as EdaDecisionEnvironment;
        privateDEId = privateDE.id;

        await expect(
          page.getByRole('heading', { name: deName, exact: true }),
        ).toBeVisible();
        ...
        await page
          .getByRole('button', { name: 'Rulebook', exact: true })
          .click();
        await page.getByRole('option', { name: edaRulebook.name }).click();

        await page
          .getByRole('button', { name: 'Decision Environment' })
          .click();
        await page
          .locator('#decision_environment_id-search input')
          .fill(deName);
        ...
        const workersAvailable =
          await RulebookActivation.api.checkWorkersAvailable(
            page,
            createdRBA.id,
          );
        if (!workersAvailable) {
        ...
        await RulebookActivation.api.waitForStatus(
          page,
          createdRBA.id,
          'failed',
        );
...
async function waitForProjectSync(
  ...
  while (attempts < maxAttempts) {
    ...
    if (projectResult?.results && projectResult.results.length > 0) {
      const currentProject = projectResult.results.find(
