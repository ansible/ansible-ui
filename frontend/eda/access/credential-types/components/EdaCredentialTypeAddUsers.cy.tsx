...
import { EdaCredentialTypeAddUsers } from './EdaCredentialTypeAddUsers';

describe('EdaCredentialTypeAddUsers', () => {
  const component = <EdaCredentialTypeAddUsers />;
  const path = '/credential-types/:id/user-access/add-users';
  const initialEntries = [`/credential-types/1/user-access/add-users`];
  const params = {
    path,
    initialEntries,
  };
  ...
  beforeEach(() => {
    cy.intercept('GET', edaAPI`/credential-types/*`, { fixture: 'edaCredentialTypes.json' });
    cy.intercept('GET', edaAPI`/users/*`, { fixture: 'edaUsers.json' });
    cy.intercept('GET', edaAPI`/role_definitions/?content_type__model=credentialtype*`, {
      fixture: 'edaCredentialTypeRoles.json',
    });
  ...
  it('should validate that at least one role is selected for moving to Review step', () => {
    ...
    cy.selectTableRowByCheckbox('name', 'Credential Type Admin', { disableFilter: true });
    cy.clickButton(/^Next$/);
  ...
  it('should display selected user and role in the Review step', () => {
    ...
    cy.get('[data-cy="expandable-section-edaRoles"]').should(
      'contain.text',
      'Has all permissions to a single credential type'
    );
  });
  it('should trigger bulk action dialog on submit', () => {
    cy.intercept('POST', edaAPI`/role_user_assignments/`, {
      statusCode: 201,
      body: { user: 5, role_definition: 14, content_type: 'eda.credentialtype', object_id: 1 },
    ...
    cy.get('.pf-v5-c-modal-box').within(() => {
      ...
      cy.get('table tbody').should('contain.text', 'Credential Type Admin');
