...
export function EditHost() {
  ...
  return (
    <PageLayout>
      <PageHeader
        title={host?.name ? t('Edit {{hostName}}', { hostName: host?.name }) : t('Host')}
        breadcrumbs={[
          { label: t('Hosts'), to: getPageUrl(AwxRoute.Hosts) },
          {
            label: host?.name ? t('Edit {{hostName}}', { hostName: host?.name }) : t('Host'),
          },
        ]}
      />
