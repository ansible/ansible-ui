...
describe('Workflow Job Templates Tests', () => {
  ...
  describe('Workflow Job Templates: Create', function () {
    it('can create a WFJT with only a name and then edit it to add all optional fields, including extra vars', () => {
      ...
      cy.verifyPageTitle('Automation Templates');
  ...
  describe('Workflow Job Templates: Edit', function () {
    ...
    it('can edit a WFJT to add, save, and then remove a webhook credential for github', () => {
      cy.createAWXCredential(
        ...
        cy.navigateTo('awx', 'templates');
        cy.verifyPageTitle('Automation Templates');
        cy.intercept('GET', awxAPI`/unified_job_templates/*`).as('result');
        cy.filterTableBySingleSelect('name', workflowJobTemplate?.name);
        cy.wait('@result');
        ...
        cy.verifyPageTitle(`Edit ${workflowJobTemplate.name}`);
        cy.get('[data-cy="isWebhookEnabled"]').should('not.be.checked');
        cy.get('[data-cy="isWebhookEnabled"]').click();
        cy.selectDropdownOptionByResourceName('webhook-service', 'GitHub');
        cy.get('button').filter(`[data-cy="webhook_credential"]`).click();
        cy.document()
          .its('body')
          .find('.pf-v6-c-menu__content')
          .within(() => {
            cy.getByDataCy('search-input').type(tokenCredential.name);
            cy.contains('.pf-v6-c-menu__item-text', tokenCredential.name).click();
          });
        cy.intercept(
          'PATCH',
          awxAPI`/workflow_job_templates/${workflowJobTemplate.id.toString()}/`
        ).as('editedWFJT');
        cy.contains('Save workflow job template').click();
        cy.wait('@editedWFJT')
          .its('response.body')
          .then((editedWFJT: WorkflowJobTemplate) => {
            expect(editedWFJT.webhook_service).contains('github');
            cy.verifyPageTitle(editedWFJT.name);
            cy.get('[data-cy="name"]').should('be.visible');
            cy.contains('Edit template').click();
            cy.verifyPageTitle(`Edit ${workflowJobTemplate.name}`);
            cy.get('[data-cy="isWebhookEnabled"]').should('be.checked');
            cy.get('[data-cy="isWebhookEnabled"]').click();
            cy.contains('Save workflow job template').click();
            cy.wait('@editedWFJT')
              .its('response.body')
