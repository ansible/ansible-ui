...
describe('DeprecationAffectedJobs', () => {
  ...
  it('should show "Showing jobs from last 7 days" by default when no timeRange param in URL', async () => {
    ...
    server.use(
      http.get(awxAPI`/jobs/`, ({ request }) => {
        const url = new URL(request.url);
        if (url.searchParams.get('id__in')) return HttpResponse.json(jobDetailsResponse);
        if (url.searchParams.get('created__gte')) return HttpResponse.json(mockJobsResponse);
        return HttpResponse.json(emptyJobsResponse);
      }),
      http.get(awxAPI`/jobs/:jobId/job_events/`, () => HttpResponse.json(mockEventsJob1))
    );

    render(<DeprecationAffectedJobs />, { wrapper: Wrapper });

    await waitFor(() => {
      expect(screen.getByText('Showing jobs from last 7 days')).toBeInTheDocument();
    });
  });

  it('should show the correct time range label when timeRange=30d is in the URL', async () => {
    const jobDetailsResponse = {
      results: [
        {
          id: 1,
    ...
    function WrapperWithTimeRange({ children }: { children: ReactNode }) {
      return (
        <SWRConfig value={{ provider: () => new Map() }}>
          <MemoryRouter
            initialEntries={[
              `/deprecations/${encodeURIComponent('with_items on module')}/affected-jobs?timeRange=30d`,
            ]}
          >
            <Routes>
              <Route
    ...
    render(<DeprecationAffectedJobs />, { wrapper: WrapperWithTimeRange });

    await waitFor(() => {
      expect(screen.getByText('Showing jobs from last 30 days')).toBeInTheDocument();
    });
  });
});
