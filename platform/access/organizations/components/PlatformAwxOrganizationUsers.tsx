...
export function PlatformAwxOrganizationUsers() {
  const { t } = useTranslation();
  const params = useParams<{ id: string }>();
  const viewAwxOrgUserRoles = useViewAwxOrgUserRoles();
  ...
  const view = usePlatformView<UserAssignment>({
    ...
    queryParams: {
      role_definition__name: 'Controller Organization Member',
  ...
  const rowActions = useMemo<IPageAction<UserAssignment>[]>(
    () => [
      {
        ...
        icon: CogIcon,
        label: t(`View roles`),
        onClick: (user: UserAssignment) => viewAwxOrgUserRoles(user),
      },
      {
        type: PageActionType.Button,
        selection: PageActionSelection.Single,
    ...
    [t, removeAssignment, viewAwxOrgUserRoles]
  );
  ...
  return (
    <PageTable<UserAssignment>
      id="platform-awx-organization-users-table"
      toolbarFilters={toolbarFilters}
      toolbarActions={toolbarActions}
      tableColumns={tableColumns}
      rowActions={rowActions}
      errorStateTitle={t('Error loading users')}
      emptyStateTitle={t('There are currently no users added to this organization.')}
