  ...
  return [
    {
      ...
      onClick: (element: WorkflowNode) => {
        dispatch({ type: 'SET_SELECTED_NODE', value: { node: [element], mode: 'edit' } });
      },
    },
    {
      key: 'add-link',
      ...
      label: t('Add link'),
      onClick: (element: WorkflowNode) =>
        dispatch({ type: 'SET_SELECTED_NODE', value: { node: [element], mode: 'add-link' } }),
    },
    {
      ...
      onClick: (element: WorkflowNode) =>
        dispatch({
          ...
          value: { node: [element], mode: 'add-node-and-link' },
        }),
    },
    {
      key: 'separator',
...
export function NodeContextMenu(
  ...
  const selectedNode = nodes.find((node: WorkflowNode) => {
    if ('id' in element && node.id.toString() === element.id) return node;
    return undefined;
  });
