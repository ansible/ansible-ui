...
describe('Schedules - Create and Delete', function () {
  ...
  it.skip('can create a simple schedule, navigate to schedule details, then delete the schedule from the details page', () => {
  ...
  it.skip('can create a simple schedule of resource type Inventory source, then delete the schedule', () => {
  ...
  it.skip('can create a simple schedule of resource type Project, then delete the schedule', () => {
  ...
  describe('Create Schedule with Prompts, Surveys and Exceptions', function () {
    ...
    it('can create a complex schedule and navigate to details page', () => {
      ...
      cy.get('[data-cy="wizard-nav"]').then(($el) => {
        if ($el.length === 6) {
      ...
      cy.wait('@scheduleCreated')
        ...
        .then((response: Schedule) => {
          ...
          expect(response.rrule).contains('RRULE:FREQ=HOURLY;INTERVAL=100;WKST=MO;COUNT=17');
          expect(response.rrule).contains('EXRULE:FREQ=YEARLY;INTERVAL=200;WKST=MO;COUNT=27');
      ...
      cy.getByDataCy('rruleset').contains('RRULE:FREQ=HOURLY;INTERVAL=100;WKST=MO;COUNT=17');
      cy.getByDataCy('rruleset').contains('EXRULE:FREQ=YEARLY;INTERVAL=200;WKST=MO;COUNT=27');
...
describe('Schedules - Edit', () => {
  ...
  it('can edit a schedule remove exceptions from row action', () => {
    ...
    cy.createAWXSchedule({
      ...
      unified_job_template: project.id,
      rrule:
        'DTSTART;TZID=America/New_York:20250430T171500 RRULE:FREQ=YEARLY;INTERVAL=1;WKST=MO;BYMONTH=4;BYMONTHDAY=30;BYHOUR=11;BYMINUTE=15;BYSECOND=0 EXRULE:FREQ=WEEKLY;INTERVAL=50;WKST=TU;BYDAY=WE;BYHOUR=11;BYMINUTE=15;BYSECOND=0',
    }).then((sched: Schedule) => {
      schedToEdit = sched;
  ...
  it('can edit a simple schedule to remove a rule and an exception', () => {
    ...
    cy.createAWXSchedule({
      ...
      unified_job_template: project.id,
      rrule:
        'DTSTART;TZID=Etc/Zulu:20240507T230000 RRULE:FREQ=WEEKLY;INTERVAL=1;WKST=MO;BYDAY=TU;BYHOUR=17;BYMINUTE=0;BYSECOND=0 EXRULE:FREQ=YEARLY;INTERVAL=1;WKST=MO;BYMONTH=5;BYMONTHDAY=7;BYHOUR=17;BYMINUTE=0;BYSECOND=0',
    }).then((sched: Schedule) => {
      schedToEdit = sched;
      cy.filterTableBySingleSelect('name', schedToEdit.name);
      cy.getBy('[data-cy="edit-schedule"]').click();
