...
export function controllerRelations(item: Record<string, any>, data: MockContext['data']) {
  if (!item) return;
  ...
  if (!item.summary_fields.labels) {
    ...
    if ('labels' in item && Array.isArray(item.labels)) {
      item.labels.forEach((labelId: unknown) => {
        ...
        const label = (data.api.controller.v2.labels as Label[]).find((l) => l.id === labelId);
        if (label) {
          (item.summary_fields.labels.results as Label[]).push(label);
        }
      });
    }
  }
